...
class Grok1MLP(nnx.Module):
    ...
    def __init__(
        ...
        self.act_fn = jax.nn.gelu #GeluAndMul(approximate="tanh")
        self.layer_id = layer_id
        self.reduce_results = reduce_results

    def __call__(self, x: jax.Array) -> jax.Array:
        gate, _ = self.act_fn(self.gate_proj(x), approximate=True) # can be used modeling_flax_utils.ACT2FN["gelu_new"]
        up, _ = self.up_proj(x)
        x, _ = gate * up
...
class Grok1Attention(nnx.Module):
    ...
    def __call__(
        ...
        q, k = self.rotary_emb(attention_metadata.input_positions, q, k)
...
class Grok1DecoderLayer(nnx.Module):
    ...
    def __call__(
        ...
        positions = attention_metadata.positions
...
class Grok1Model(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Analyze Embed class and see if its same as nnx.Embed
        ...
        # Mesh isn't  provided yet, if it creates problem, need to switch
    ...
    def __call__(
        ...
        attention_metadata: AttentionMetadata,
    ) -> tuple[jax.Array, list[jax.Array], list[jax.Array | None]]: # Need to work on the output of the method
        ...
        layers_kv_fused = []
        layers_topk_ids = []
        for i, layer in enumarate(self.layers):
        ...
        return hidden_states, kv_caches, layers_topk_ids
...
class Grok1ForCausalLM(nnx.Module):
    ...
    def __init__(
        ...
        super().__init__()
        dtype == jnp.bfloat16 # Removing assertion and hardcoding bf16
        self.vllm_config = vllm_config
        config = vllm_config.model_config.hf_config # Added to access hf config in the vllm config
        ...
        # Main model
        self.model = Grok1Model(vllm_config=vllm_config, rng=self.rng,  mesh=mesh)

        # Language modeling head (matching PyTor
        # Might need to access config.model_config.hf_config for getting the PreTrainedConfig https://github.com/vllm-project/vllm/blob/ab374786c7ebc7e3b6af1778404094261338e4ca/vllm/config/model.pych logic for replicated vs parallel)
